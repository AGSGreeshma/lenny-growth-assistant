...
7. **Ollama request timeout** raised from a guessed 60s (which failed real requests) to a measured 180s, based on live timing across all three generation paths. *(Superseded by #15 below: later revised to a 120s soft streaming budget as a deliberate latency/length trade-off, not a regression back to the original guess.)*
...
15. **Ship 30 for 30 length vs. local-model latency — deliberate soft-deadline trade-off.** `OllamaClient` switched from a single buffered response to a streamed one with a soft wall-clock budget (`OLLAMA_TIMEOUT_SECONDS`, revised to `120s`): if the budget runs out, whatever coherent content has been generated is trimmed and returned instead of being discarded, and only a cutoff too short to be useful is treated as a failure (`GenerationTimeoutError`, mapped to a `504`). The Ship 30 prompt was rewritten to rank groundedness/coherence above exact word count and explicitly forbid padding. See `docs/architecture.md`'s "Ship 30 for 30 length vs. local-model latency" section and `agent_transcripts/13` for the full reasoning and the live measurements behind it.

...
3. **Generate an essay** → user requests a Ship 30 for 30-style writeup (either via
   ...
   grounded material into a ~1,250-word essay → rendered as a Markdown artifact in
   the side panel.
...
| 3 (operational) | **Local (Ollama) Generation Latency** | Wall-clock time from request received to response returned, on the local/CPU-only path. | Proposed target: comfortably inside a configurable timeout (`OLLAMA_TIMEOUT_SECONDS`, default 180s). | **Observed** live during this engagement, end-to-end through the real API (not mocked), on CPU-only reference hardware with `llama3.2:3b`: plain chat ~64s, Ship 30 essay ~78-105s, HTML artifact ~121-150s. These numbers are why the timeout default was raised from an initial 60s (which failed real requests) to 180s. |
...
| **Latency** | CPU-only local inference can take over a minute per response, especially for longer skills (essay, HTML artifact). | `OLLAMA_TIMEOUT_SECONDS` was set from live measurement (300s, re-measured after a GPU/CUDA driver failure on one machine forced CPU-only inference there — essay/HTML measured 198.6s/180.5s) rather than guessed; automatic OpenAI fallback exists when configured. | On genuinely slow hardware, even 300s may not be enough; with no cloud key configured, a timeout there is a hard failure, not a degraded answer. |
...
| **Model timeout** | A slow local response is indistinguishable from a hung one without a bound, and a too-short bound fails otherwise-successful requests. | `OLLAMA_TIMEOUT_SECONDS` is configurable and was tuned from live, end-to-end measurement across all three generation paths (not guessed). | Any fixed timeout is a bet against variable hardware; slower machines than the reference one used here may still need a higher value. |
